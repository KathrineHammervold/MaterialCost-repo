...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('http://127.0.0.1:5000/get_house_parts')
        ...
        // Add event listener to calculate the total cost when the button to calculate total cost is clicked
        const calculateTotalCostButton = document.getElementById('CalculateTotalCost');
        calculateTotalCostButton.addEventListener('click', function() {
            calculateTotalCost();   
        });
...
function fetchSupplier(sizeCell, material, supplierCell, costPerUnitCell, totalCostCell) {
    ...
    // Function to calculate the total cost based on all the values in the Total Cost column
function calculateTotalCost() {
    console.log('Calculating total cost for all rows');
    const tableBody = document.getElementById('dataTable').getElementsByTagName('tbody')[0];
    let totalCost = 0;
    for (let row of tableBody.rows) {
        const totalCostCell = row.cells[5];
        const totalCostValue = parseFloat(totalCostCell.textContent);
        if (!isNaN(totalCostValue)) {
            totalCost += totalCostValue;
        }
    }
    console.log('Total cost for all rows:', totalCost);
    totalCostDisplay = document.getElementById('totalCostDisplay');
    totalCostDisplay.textContent = totalCost.toString(); 
    // You can display the total cost in a specific element if needed
}